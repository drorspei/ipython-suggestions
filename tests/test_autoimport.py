...
def ip(global_ip):
    ...
    global_ip.run_cell("%load_ext ipython_suggestions")
    yield global_ip
...
def test_sub_submodule(ip):
    ...
    assert (captured.stdout == "suggestions: import a.b.c\nOut[1]: 'a.b.c'\n")
...
def test_setattr(ip):
    ...
    assert captured.stdout == "suggestions: import a\nOut[1]: (True, 42)\n"
...
def test_unload(ip):
    with IPython.utils.io.capture_output() as captured:
        ip.run_cell("%unload_ext ipython_suggestions")
